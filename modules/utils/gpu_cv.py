...
# ═══════════════════════════════════════════════════════════════════
#  KHÔNG wrap những hàm sau vì chi phí upload/download > lợi ích GPU:
#  - cv2.bitwise_and()  → thao tác đơn giản trên mask, CPU nhanh hơn
#  - cv2.addWeighted()  → thường dùng trên ảnh nhỏ (overlay text)
#  - cv2.rectangle()    → vẽ hình, không có CUDA equivalent
#  - cv2.putText()      → vẽ chữ, không có CUDA equivalent
#  - cv2.Canny()        → thường dùng trên ảnh nhỏ (crop biển số)
#  - cv2.dilate()       → thường dùng trên ảnh nhỏ (crop biển số)
# ═══════════════════════════════════════════════════════════════════
